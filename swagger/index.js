...
// Function to dynamically load all feature JSON files
const loadFeatures = () => {
  ...
  let combinedSwagger = {
    ...
    info: {
      ...
      description: "API documentation for app features",
    },
    servers: [
      {
  ...
  featureFiles.forEach((file) => {
    ...
    // Merge each feature's data into the combined Swagger document

    combinedSwagger.tags.push(...(featureData.tags || []));
    combinedSwagger.paths = _.merge(
      combinedSwagger.paths,
      featureData.paths || {}
    );
    combinedSwagger.components.schemas = _.merge(
      combinedSwagger.components.schemas,
      featureData.components?.schemas || {}
    );
    combinedSwagger.components.responses = _.merge(
      combinedSwagger.components.responses,
      featureData.components?.responses || {}
    );
  });
