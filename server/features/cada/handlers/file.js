...
const cadaFileServices = require("../services/file");
...
exports.processWfdb = (req, res) => {
  function runScript() {
    return spawn(process.env.PYTHON_PATH, [
      path.join(__dirname, "../../../python-scripts/wfdb-handler.py"),
      path.join(process.env.BUCKET_PATH || "", `${req.query.filename}`),
      req.query.offset,
      req.query.range,
    ]);
...
exports.processText = (req, res) => {
  const filePath = path.join(process.env.BUCKET_PATH || "", `${req.query.filename}`);
...
exports.bulkCreate = async (req, res) => {
  ...
  const uniqueFiles = [...new Set(files.map(JSON.stringify))].map(JSON.parse);

  try {
    const filePaths = uniqueFiles.map((f) => f.path);
    const existing = await cadaFileServices.findByFiles(filePaths);
    const existingPaths = existing.map((e) => e.get({ plain: true }).path);
    let newFiles = uniqueFiles.filter((file) => !existingPaths.includes(file.path));
    if (newFiles?.length === 0) {
      return res.status(200).json(existing);
    }
    // Bulk create new files
    await cadaFileServices.bulkCreate(newFiles);
    // Fetch all files after creation
    const allFiles = await cadaFileServices.findByFiles(filePaths);
    return res.status(200).json(allFiles.map((f) => f.get({ plain: true })));
  } catch (err) {
