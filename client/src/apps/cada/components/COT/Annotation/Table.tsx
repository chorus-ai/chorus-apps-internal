...
export default function TokenTable({ headers, tokens, otherTokens }: { headers: any; tokens: any; otherTokens: any }) {
        ...
        <Table size="small" aria-label="simple table" style={{ backgroundColor: "transparent" }}>
          ...
          <TableBody>
            {tokens?.tokens?.length > 0 ? (
              tokens.tokens.map((token: any, i: any) => {
                return (
                  <TableRow key={i} sx={{ background: otherTokens.tokens.filter((t: any) => t.token === token.token).length > 0 ? "linear-gradient(90deg, rgba(2,0,36,0) 0%, rgba(230,255,240,0.5) 16%, rgba(230,255,240,0.8) 37%, rgba(230,255,240,0.8) 62%, rgba(230,255,240,0.5) 80%, rgba(0,212,255,0) 100%)" : "transparent" }}>
                    {headers.map((header: any, j: any) => {
                      if (header.name === 'rr') {
                        return (
                        ...
                        <TableCell key={j} sx={{ maxWidth: '10%', width: "10%" }}>
                          {
                            header.name === 'tokens' ?
                              ...
                              >
                                <Label
                                  color={otherTokens.tokens.filter((t: any) => t.token === token.token).length > 0 ? 'success' : 'info'}
                                  sx={{
                                    userSelect: "none",
