...
function splitTextByTags(text: any) {
  const regex = /(<[A-Z_]*>)/g; // Matches everything wrapped in <>
  return text.split(regex).map((part: any) => ({
    text: part.replace(/(<|>)+/g, ''),
...
function applyHighlights(text: any, highlights: any) {
...
const HighlightText = ({ text, highlights, selectedConcept, onHighlightClick, values, ___tags }) => {
