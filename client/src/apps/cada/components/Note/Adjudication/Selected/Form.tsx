...
export default function Form({ cIdx, value, setValue, event }) {
  ...
  const blockUids = [1, 2, 171]

  const filteredAnnotations = event.cadaAnnotations.map((a: any) => {
  ...
  }).filter((a: any) => !blockUids.includes(a.userId));

  const annotationUsers = (option: any, cid: any, question: any) => {
    let users = [];
  ...
  const avatarGroup = (option: any, cid: any, question: any) => {
    return (
      <AvatarGroup max={4}>
  ...
  const findTheOnlyMaxNumber = (arr: any) => {
    let max = 0;
    let maxIdx = 0;
    let maxCount = 0;
