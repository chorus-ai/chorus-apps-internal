...
export default function Overview({ ____project, events, selectedIds, setSelectedIds, handleClickAdjudicate }) {
  const [_annotatedNotes, _setAnnotatedNotes] = useState({});
  const [_adjudicatedNotes, _setAdjudicatedNotes] = useState({});
  const [_annotatedConcept, _setAnnotatedConcept] = useState({});
  const [annotationResult, _setAnnotationResult] = useState({});
  const [noteIdx, _setNote] = useState(0);
  const [_conceptIdx, _setConcept] = useState(0);
  const [_detected, _setDetected] = useState("");
  const [_encounter, _setEncounter] = useState("");
  const [_negated, _setNegated] = useState("");
  ...
  const ____annotators = useAppSelector((state) =>
                              ...
                              >
                                ...
                                <TableBody>
                                  {notes[row.file.split(".")[0]]
                                    && notes[row.file.split(".")[0]].info.map((conceptRow: any, i: any) => (
                                      <TableRow key={i}>
                                        <TableCell
                                          component="th"
                                          scope="row"
