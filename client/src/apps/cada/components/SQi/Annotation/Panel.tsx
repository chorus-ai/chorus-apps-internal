...
function Annotation({
  ...
  const [buttonsDisabled, _setButtonsDisabled] = React.useState(false);
  const [value, setValue] = React.useState(0);
  const [_openAlert, _setOpenAlert] = React.useState(false);
  ...
  const [_left, _setLeft] = React.useState(0);
  ...
  const handleSubmit = (e: any) => {
    ...
    saveAnnotation.mutate({
      ...
      eventId: events[String(completedTab)][event].id,
      completed: completedTab,
      annotation: {
        field: e.currentTarget.value,
        value: JSON.stringify(selectedRanges),
        cadaAnnotationId: events[String(completedTab)][event].cadaAnnotations[0].id,
  ...
  const handleLabelDelete = (index: any) => () => {
    setSelectedRanges([
  ...
  useEffect(() => {
    ...
    if (chartObjects) {
      let charts = chartObjects;
      for (let i = 0; i < charts.length; i++) {
        let ____temp = Object.assign({}, charts[i]);
        charts[i].axisX[0].margin = 260;
        charts[i].sessionVariables.axisX[0].newViewportMinimum = 0;
        charts[i].sessionVariables.axisX[0].newViewportMaximum =
          events[String(completedTab)][event].cadaFile.Length;
  ...
  useDidMountEffect(() => {
    if (value === 1) {
      let annotationObj =
        events[String(completedTab)][event].cadaAnnotations[0].cadaAnnotationValues;
      let maxIdAnnotationValue = annotationObj.reduce((prev: any, curr: any) =>
  ...
  if (
    ...
    events[String(completedTab)][event] &&
    events[String(completedTab)][event].cadaAnnotations &&
    events[String(completedTab)][event].cadaAnnotations[0].cadaAnnotationValues
  ) {
    return (
        ...
        <Toolbar disableGutters={true}>
          ...
          {completedTab
            ? JSON.parse(project.attributes).Buttons.map((b: any, _i: any) => {
                let latestElement =
                  events[String(completedTab)][event].cadaAnnotations[0]
        ...
        <div style={{ ...useStyles.demo1 }}>
          <Toolbar style={{ ...useStyles.toolbar }}>
            ...
            <Tooltip
              title={
                `Filename: ` +
                events[String(completedTab)][event].cadaFile.path.split("/").pop()
              }
            >
          ...
          {events[String(completedTab)].length > 0 ? (
            <Chart
              ...
              range={30}
              fileLength={30}
              filename={events[String(completedTab)][event].cadaFile.path}
              nextFilename={
                typeof events[String(completedTab)][event + 1] === "undefined"
                  ? ""
                  : events[String(completedTab)][event + 1].cadaFile.path
              }
            />
          ) : (
            ""
