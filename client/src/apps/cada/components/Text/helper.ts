...
export const combineOffsets = (offsets: any, modelValues: any) => {
  ...
  for (let offset of offsets) {
    ...
    const overlappingModels = modelValues.filter(
      (model: any) => Number(model.start) < end && Number(model.end) > start
    );
...
export const splitWithOffsets = (text: any, offsets: any) => {
  let lastEnd = 0;
  const splits = [];
...
export const selectionIsBackwards = (selection: any) => {
