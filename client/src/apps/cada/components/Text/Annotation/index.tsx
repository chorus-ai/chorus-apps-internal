...
import { getAnnotationEvents, updateAnnotation } from "../../../store/thunk";
  ...
  for (let offset of offsets) {
    ...
    if (overlappingModels.length > 0) {
      combinedOffsets.push({
        ...
        models: overlappingModels.map((model: any) => ({
          ...model,
          start: Number(model.start),
          end: Number(model.end),
        })),
...
export default function Annotation({ pid }) {
  ...
  const getNoteContent = async (events: any, page: any) => {
    ...
    if (currentEvent.cadaAnnotations[0]?.cadaAnnotationValues.length > 0) {
      ...
      const sortedAnnotationValues = currentEvent.cadaAnnotations[0].cadaAnnotationValues.sort((a: any, b: any) => b.id - a.id);
    ...
    if (result.data.info && result.data.info.length > 0) {
      ...
      .filter((item: any) => parseFloat(Number(item[3]).toFixed(2)) > sliderValue)
      .map((item: any) => {
        return {
          tag: item[0],
          start: item[1],
              ...
              <Grid>
                <Download
                  data={Object.entries(state).flatMap(([key, value]) =>
                    value.map((childObj: any) => ({ ...childObj, noteIndex: [...events[true], ...events[false]][key].cadaFile.path })) 
                  )}
                  filename="data.csv"
                  style={{
                    border: "none",
