...
function getCompleted(annotations: any) {
  let completed = 0;
...
function getConcepts(annotations: any) {
  let concepts = {};
  let temp = {};
  for (const element of annotations) {
...
function calculateNoteAgreement(note: any) {
  let percentage = 0;
  let temp = [];
...
function isAdjudicated(noteAdj: any) {
  if (noteAdj.length > 0) {
...
function getConceptValueRows(values: any, agreement: any) {
  return (
    <React.Fragment>
...
function Overview({
  ...
  const ____params = useParams();

  const [_annotatedNotes, _setAnnotatedNotes] = React.useState({});
  const [_adjudicatedNotes, _setAdjudicatedNotes] = React.useState({});
  const [_annotatedConcept, _setAnnotatedConcept] = React.useState({});
  const [annotationResult, _setAnnotationResult] = React.useState({});
  const [noteIdx, _setNote] = React.useState(0);
  const [_conceptIdx, _setConcept] = React.useState(0);
  const [_detected, _setDetected] = React.useState("");
  const [_encounter, _setEncounter] = React.useState("");
  const [_negated, _setNegated] = React.useState("");
  ...
  const [_tab, setTab] = React.useState(0);

  const annotators = useAppSelector((state) =>
    state.cada.annotators ? state.cada.annotators : null
  );

  const ____handleChange = (_e: any, newValue: any) => {
    setTab(newValue);
  };

  const handleRequestSort = (_e: any, property: any) => {
  ...
  const handleSelectAllClick = (e: any) => {
    if (e.target.checked) {
      const newSelecteds = events.map((n: any, _i: any) => n.id);
  ...
  const handleCheckboxClick = (_e: any, name: any) => {
  ...
  const handleExpandClick = (_e: any, name: any) => {
  ...
  const handleChangePage = (_e: any, newPage: any) => {
    setPage(newPage);
  };
  ...
  const handleChangeDense = (e: any) => {
    setDense(e.target.checked);
  };

  const isSelected = (name: any) => selectedIds.indexOf(name) !== -1;
  const isExpanded = (name: any) => expanded.indexOf(name) !== -1;
  ...
  const rows = events.map((note: any) =>
    createData(
      note.id,
