...
const ____SquareIconButton = styled(IconButton)(({ theme }) => ({
...
function isAssigmentComplete(fieldIdsSet: any, target: any) {
...
function removeStartsWith(str: any, substringToRemove: any) {
...
export default function NlpNotes({
  ...
  const handleSave = () => {
    ...
    saveAnnotation.mutate({
      projectId: project.id, eventId: events[eIdx].id, completed: completedTab, annotation: {
  ...
  const handleHighlightClick = (conceptIndex: any) => {
  ...
  const handleDetected = (e: any) => {
    setDetected(e.target.value);
  };

  const handleEncounter = (e: any) => {
    setEncounter(e.target.value);
  };

  const handleNegated = (e: any) => {
  ...
  const [_activeChipIndex, setActiveChipIndex] = useState(0);

  const ____handleNextChip = () => {
  ...
  const [_selectedChips, setSelectedChips] = useState({});

  const ____handleChipClick = (category: any, label: any) => {
