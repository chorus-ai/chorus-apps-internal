...
export default function Panel({ project, eIdx, setEIdx, events }: { project: any; eIdx: any; setEIdx: any; events: any }) {
  ...
  const handleSave = () => {
    ...
    const isComplete = values.every((value: any) => Object.keys(value).length === content.info.length && !Object.values(value).includes(null));
  ...
  const setter = (idx: any, model: any) => (value: any) => {
    setValues(curr => {
      const tmp = [...curr];
      tmp[idx][model] = value;
      return tmp;
            ...
            <Grid>
              ...
              >
                ...
                {content.info && (
                  ...
                  >
                    <Grid sx={{ fontWeight: 'bold' }} size="grow">
                    ...
                    {content.info[questionIdx] && Object.keys(content.info[questionIdx].LLMs).map((llm, index) => {
                      return (<div key={index}>
                        <Divider />
                        <Grid sx={{ mt: 1, fontWeight: 'bold' }} size="grow">
                          {llm}:
                        </Grid>
                        <Grid container direction="row" spacing={3} padding={2}>
                          <Grid xs={questionIdx >= content?.info.length - 2 ? 6 : 12}>
