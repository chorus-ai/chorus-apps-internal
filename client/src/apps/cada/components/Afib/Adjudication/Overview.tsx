    ...
    >
      ...
      {numSelected > 0 ? (
        <FormControlLabel
          ...
          control={
            <MdKeyboardArrowRight name="adj" size={25} />
          }
          label={
            <Typography
              style={{ flex: "1 1 100%" }}
...
function getAgreement(ann: any, id: any) {
  return (
...
function getReliability(ann: any) {
  let percentage = 0;
  let max = 0;
  let sum = 0;
...
function getAdjudicationValue(noteAdj: any) {
  if (noteAdj.length > 0) {
    let maxId = noteAdj.length > 0 ? noteAdj[0].id : null;

    noteAdj.forEach((adjudicationValue: any) => {
...
export default function Overview({
  ...
  if (events && Object.keys(annotatedRecords).length > 0) {
    const rows = events.map((record: any) =>
      createData(
        record.id,
