...
const CustomWidthTooltip = styled(({ className, ...props }: TooltipProps & { className?: string }) => (
  <Tooltip {...props} classes={{ popper: className }} />
))({
  [`& .${tooltipClasses.tooltip}`]: {
    maxWidth: 500,
...
export default function Selected({
  ...
  const annotators = useAppSelector((state) =>
    Object.keys((state.cada as any).annotators).length > 0 ? (state.cada as any).annotators : null
  );
  ...
  const handleSubmit = (e: any) => {
    ...
    dispatch(
      updateAdjudication(
        project.id,
        events[String(completedTab)][eIdx].id,
        user?.id,
        completedTab,
        {
          field: project.name,
          value: e.currentTarget.value,
          userId: user?.id,
          cadaEventId: events[String(completedTab)][eIdx].id,
  ...
  const getChartObj = (obj: any, type: any) => {
    if (type === "add") {
      ...
      setChartObjects((chartObjects) => {
        let copy: any[] = Object.assign([], chartObjects);
        ...
        <Toolbar disableGutters>
          ...
          {completedTab
            ? JSON.parse(project.attributes).Buttons.map((b: any, _i: any) => {
                let latestElement =
                  events[String(completedTab)][eIdx].cadaAdjudicationValues.length > 0
                    ? events[String(completedTab)][eIdx].cadaAdjudicationValues.sort(
            ...
            <CustomWidthTooltip
              title={
                events[String(completedTab)][eIdx]
                  ? `Filename: ` + events[String(completedTab)][eIdx].cadaFile.path.split("/").pop()
                  : ""
              }
            >
              <IconButton color={"default" as any}>
                <MdInfo />
              </IconButton>
            </CustomWidthTooltip>
          </Toolbar>
