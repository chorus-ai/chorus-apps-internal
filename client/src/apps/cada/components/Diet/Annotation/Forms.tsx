...
export default function Forms({ values, setValues, content }: { values: any; setValues: any; content: any }) {

  const onNutritionChange = (key: any) => (e: any) => {
    setValues((curr: any) => ({ ...curr, nutrition: { ...curr.nutrition, [key]: e.target.value } }));
  }

  const onIngredientChange = (e: any) => {
    setValues((curr: any) => ({ ...curr, ingredients: e.target.value }));
  }

  const onCommentChange = (e: any) => {
    setValues((curr: any) => ({ ...curr, comment: e.target.value }));
  }
  ...
  return (
    <>
      ...
      >
        <Box>
          The ingridents in this meal are: {content?.ingredients?.split(',')?.map((ing: any, index: any) => (
        ...
        {content?.GPTFoodCode && Object.keys(content.GPTFoodCode).map((key, index) => (
          <div key={key}>
            ...
            <Box>
              {content.GPTFoodCode[key].length > 0 ? content.GPTFoodCode[key].map((code: any, index: any) => (
                <Tooltip
                  key={index}
                  title={code.code}
