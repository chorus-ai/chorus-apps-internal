...
//
function calculateConceptAgreement(values: any) {
  let max = 0;
  let sum = 0;
...
//
export const calculateAnnotationValues = (annotations: any) => {
  let organizedValues = {};
...
export const getAnnotatedEvents = (events: any) => {
  let ann = {};
...
export const getAdjudicatedEvents = (events: any) => {
  let adj = {};
  for (let i = 0; i < events.length; i++) {
    adj[i] = {};
