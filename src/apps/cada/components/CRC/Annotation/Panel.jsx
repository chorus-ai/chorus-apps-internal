...
export default function Panel({ project, eIdx, setEIdx, events }) {
  ...
  const handleSave = () => {
    const isComplete = values.every(
      (value) =>
        Object.keys(value).length === content.info.length &&
  ...
  useEffect(() => {
    ...
    axios({
      ...
      .then((res) => {
        ...
        const tmp_dict = models.map((model) => {
          return { [model]: null };
        });
        const tmp = res.data.info.map(() => {
          return {};
        });
        const annotation = filterMostRecentByField(
          events[eIdx].cadaAnnotations[0].cadaAnnotationValues
        );
        for (let annotationValues of annotation) {
          ...
          tmp[field] = Object.keys(value).includes(models[0])
            ? value
            : tmp_dict;
        }
  ...
  const canSave = () => {
    if (values[questionIdx] && originalValues[questionIdx]) {
      return (
        !isEqual(values[questionIdx], originalValues[questionIdx]) &&
        Object.keys(values[questionIdx]).length !== 0 &&
        !Object.keys(values[questionIdx]).every((key) =>
          Object.values(values[questionIdx][key]).every(
            (value) => value === null || value === ""
          )
        ) &&
        !Object.values(values[questionIdx]).every(
          (value) =>
            value === null || value === "" || Object.keys(value).length === 0
        )
      );
    }
                    ...
                    <div className="btns">
                      ...
                      <Button
                        ...
                        disabled={
                          ...
                          questionIdx === content?.info?.length - 1
                        }
                        onClick={handleNext}
                      >{`${canSave() ? "Save & " : ""}Next`}</Button>
                    </div>
                  </Stack>
                </Paper>
                {content.info && (
                  ...
                  >
                    <Grid item sx={{ fontWeight: "bold" }} xs>
                      {`${questionIdx + 1}. ${content.info[
                        questionIdx
                      ]?.prompt.replace("{}", "")}`}
                    </Grid>
                    {content.info[questionIdx] &&
                      Object.keys(content.info[questionIdx].LLMs).map(
                        (llm, index) => {
                          return (
                            <div key={index}>
                              <Divider />
                              ...
                              >
                                ...
                                >
                                  <Markdown>
                                    {
                                      content.info[questionIdx].LLMs[llm]
                                        .response
                                    }
                                  </Markdown>
                                  {questionIdx >= content?.info.length - 2
                                    ? null
                                    : `(Word count: ${
                                        content.info[questionIdx].LLMs[
                                          llm
                                        ].response.split(" ").length
                                      }, FKG score: ${
                                        content.info[questionIdx].LLMs[llm].FKG
                                      })`}
                                </Grid>
                                {questionIdx >= content.info.length - 2 && (
                                  <Grid
                                  ...
                                  >
                                    <Markdown>
                                      {content.info[1].LLMs[llm].response}
                                    </Markdown>
                                  </Grid>
                                )}
                              </Grid>
